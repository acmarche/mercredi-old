...
class PyaerTest extends BaseUnit
{
    ...
    {
        $jour = $this->em->getRepository(Jour::class)->findOneBy(array('date_jour' => new \DateTime($date)));
        $enfant = $this->em->getRepository(Enfant::class)->findOneBy(array('slugname' => $this->urlLisa));
        $tuteur = $this->em->getRepository(Tuteur::class)->findOneBy(array('slugname' => $this->slugNatacha));
